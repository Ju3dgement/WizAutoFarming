...
async def main(sprinter):
  ...
  while True:
    ...
    await asyncio.gather(*[player.teleport(XYZ(-37, 13154, 4.5)) for player in clients]) # Outside dungeon
    await asyncio.sleep(3)
    ...
    await asyncio.gather(*[player.teleport(XYZ(-27, 57, 0))for player in clients]) # Gather health/mana
    print ("Finished Run")
    await asyncio.sleep(3)
    await asyncio.gather(*[player.teleport(XYZ(16.8, -1508, 0))for player in clients])
