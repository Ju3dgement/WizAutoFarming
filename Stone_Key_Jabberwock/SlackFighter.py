...
from utils import is_control_grayed
...
class SlackFighter(CombatHandler):
		...
		while discarded:
			...
			for c in self.cards.copy():
				if all([self.card_names[c] not in s for s in master_list]):
					...
					await c.discard(sleep_time=0.35)
					...
					if e in self.spell_logic and c in self.enchant_logic:
						if self.spell_logic[e] == self.enchant_logic[c]:
							...
							if await self.enchant_logic_stacking(e, c, selected):
								...
								await e.cast(c, sleep_time=0.35)
								_ = await self.get_valid_cards(self.card_exclusions)
								await self.assign_card_names()
								await self.assign_spell_logic()
								await self.assign_pip_values()
	...
	async def is_control_grayed(win: Window):
	...
	async def get_tc(self):
		if self.tc:
			...
			if num_cards > 0:
				...
				if await is_control_grayed(draw_button) == False:
		...
		while discarded:
			...
			if to_discard:
				for i in to_discard:
					...
					await i.discard(sleep_time=0.35)
				_ = await self.get_valid_cards(self.card_exclusions)
				await self.assign_card_names()
				await self.assign_spell_logic()
				await self.assign_pip_values()
		...
		while True:
			if self.selected_spell:
				...
				if 'AOE' in selected_spell_logic:
					...
					await self.selected_spell.cast(None, sleep_time=0.35)
					...
					await self.selected_spell.cast(self.selected_enemy, sleep_time=0.35)
					...
					await self.selected_spell.cast(self.selected_ally, sleep_time=0.35)
